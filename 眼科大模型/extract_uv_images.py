...
"""
从葡萄膜炎病例中提取OCT和FFA图像
基于 extract_csc_images.py 修改而来:
- 数据根目录: /share/kongzitai/ophthalmology/E:/<year>
- 使用 Excel: dataset/截图病例汇总/葡萄膜炎除重后名单（全）11.23(2).xlsx
- 只处理 sheet: "科林" 和 "视微"
...
def process_single_case(row, oct_type, output_base_dir, year):
    ...
    if not all_oct_folders and oct_date:
    ...
    if all_oct_folders:
        ...
        # 提取策略
        if force_blood_flow_only:
            # 误标为普通科林, 实际应从血流模式读取
            if blood_flow_folders:
                oct_folders.extend(blood_flow_folders)
            else:
                oct_folders.extend(normal_folders)
                result['errors'].append('标记为误标科林血流模式, 但未找到血流模式文件夹, 使用普通OCT代替')
        else:
            # 正常策略: 优先普通OCT, 如有需要可同时包含血流模式
            if normal_folders:
                oct_folders.extend(normal_folders)
            if blood_flow_folders:
                # 对于葡萄膜炎任务, 血流信息也有价值, 全部提取
                oct_folders.extend(blood_flow_folders)
...
def main():
    ...
    input_excel = Path('/data2/xuhanyang/dataset/截图病例汇总/葡萄膜炎除重后名单（全）11.23(2).xlsx')
    output_base_dir = Path('/data2/xuhanyang/dataset/葡萄膜炎病例图像提取')

    sheets = ['科林', '视微']

    print("="*70)
    print("葡萄膜炎 - OCT和FFA图像提取工具")
    print("="*70)
    ...
    report_path = output_base_dir / "提取报告_葡萄膜炎.txt"
