...
# -*- coding: utf-8 -*-
"""
智能分割OCT图像中的各个子图 - 修改版，直接处理指定的OCT文件夹
"""
...
def main():
    ...
    if len(sys.argv) > 1:
        # 直接处理指定的OCT文件夹
        oct_folder = Path(sys.argv[1])
        if not oct_folder.exists():
            print(f"错误: OCT文件夹不存在 - {oct_folder}")
            return
        print(f"处理指定的OCT文件夹: {oct_folder}")
        process_oct_folder(oct_folder)
        print("\n" + "="*70)
        print("全部完成!")
        print("="*70)
    else:
        print("使用方法: python split_oct_specific.py <OCT文件夹路径>")
        print("示例: python split_oct_specific.py /path/to/OCT")
