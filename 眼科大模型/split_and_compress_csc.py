...
# -*- coding: utf-8 -*-
"""
将CSC病例图像提取文件夹分成多个部分进行压缩，每个压缩包小于1GB
包含FFA、OCT和欧宝图片
"""
...
def split_and_compress(source_dir, output_dir, max_size_gb=1.0, archive_base_name=None):
    """
    将源目录分成多个部分并压缩
    ...
    Args:
        ...
        archive_base_name: 压缩包基础名称（如果不指定，使用源目录名称）
    """
    source_dir = Path(source_dir)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # 如果没有指定压缩包基础名称，使用源目录名称
    if archive_base_name is None:
        archive_base_name = source_dir.name
    ...
    for idx, group in enumerate(groups, 1):
        ...
        # 压缩：直接压缩，不复制文件
        archive_name = f"{archive_base_name}_part{idx:02d}.tar.gz"
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 3:
        ...
        # 默认CSC代码
        source_dir = Path('/data2/xuhanyang/dataset/截图文件汇总/CSC病例图像提取')
        output_dir = Path('/data2/xuhanyang/dataset/截图文件压缩包（含欧宝）')
