...
def remove_nonstandard_oct(base_dir, target_size=(1264, 496), dry_run=False):
    """删除非标准规格的OCT图像"""
    ...
    print(f"目标规格: {target_size[0]}x{target_size[1]}")
...
def main():
    parser = argparse.ArgumentParser(description="删除非标准规格（非1264x496）的OCT图像")
    parser.add_argument("--input", "-i", default="/data2/xuhanyang/dataset/UV_CSC找不到名单图像提取",
                        help="输入目录路径")
    parser.add_argument("--size", default="1264x496",
                        help="目标规格，格式为 '宽x高' (默认: 1264x496)")
    ...
    try:
        ...
        print(f"错误: 无效的规格格式 '{args.size}'，应为 '宽x高'")
        return 1
    
    stats = remove_nonstandard_oct(args.input, target_size, args.dry_run)
    ...
    if stats['removed_files']:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write("="*70 + "\n\n")
            f.write(f"目标规格: {target_size[0]}x{target_size[1]}\n")
