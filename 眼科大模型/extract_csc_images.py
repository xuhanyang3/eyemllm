...
def parse_visit_date(date_str):
    """
    解析就诊日期
    例如: "2024-12-14 单眼" -> ("2024-12-14", "单眼")
    """
...
def find_exam_folders(base_path, patient_folder, visit_date, exam_keyword):
    ...
    Args:
        ...
        patient_folder: 患者文件夹字符串 (例如: "02036873 陈康炎 男 1973-11-11")
        visit_date: 就诊日期 (例如: "2024-12-14")
        exam_keyword: 检查关键词 (例如: "科林", "造影", "视微")
    ...
    for base in possible_bases:
        ...
        # 直接在base下查找患者文件夹（中心性浆液性脉络膜视网膜病变目录结构）
...
def find_nearest_date_oct_folders(patient_folder, visit_date, oct_keywords=['科林', '视微', '海德堡', 'OCT']):
    """
...
def process_single_case(row, oct_type, output_base_dir, year):
    ...
    # 解析patient_folder
    if 'patient_folder' in row:
        patient_folder = row['patient_folder']
    else:
        # 2025年的文件使用不同的列名
        patient_folder = row[' gΩ'] if ' gΩ' in row else row.iloc[0]
...
def process_excel_file(excel_path, oct_type, output_base_dir):
    ...
    print(f"OCT类型: {oct_type}")
    print(f"年份: {year}")
    print(f"{'='*70}\n")
    
    # 读取指定sheet
    try:
        df = pd.read_excel(excel_path, sheet_name=oct_type, engine='openpyxl')
    except Exception as e:
        print(f"错误: 无法读取sheet '{oct_type}' - {e}")
        return []
...
def main():
    ...
    # 使用新的输出目录，避免覆盖旧结果
    output_base_dir = Path('/data2/xuhanyang/dataset/DR病例图像提取')
    
    # Excel文件列表
    excel_files = [
        '2024-糖尿病性视网膜病变-单双眼.xlsx',
        '2025-糖尿病性视网膜病变-单双眼.xlsx',
    ]
    ...
    for excel_file in excel_files:
        ...
        for oct_type in oct_types:
            results = process_excel_file(excel_path, oct_type, output_base_dir)
